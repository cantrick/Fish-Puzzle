...
function findMoveMatch() {
	//returns x and y values for the grid, and the direciton which the match goes:
	//i = y
	//j = x
	...
	for(i=0;i<8;i++) {
		...
		for(j=0;j<8;j++) {
			...
			for(k=j;k<7;k++) {
				if(ds_grid_get(global.bGrid,k,i) == ds_grid_get(global.bGrid,k+1,i)) {
					combo++;
				} else {
					show_debug_message("Combo was: " + string(combo) + " on line: " + string(i));
					combo = 0;
				}
